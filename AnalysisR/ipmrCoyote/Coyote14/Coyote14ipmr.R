...
all_data<- read.csv("ASESORADOS/Erasmo/IPM2019/Marzo2019/Coyote/Coyote14.csv",
...
min.size=.9*min(c(all_data$size,all_data$sizeNext),na.rm=T)
max.size=1.1*max(c(all_data$size,all_data$sizeNext),na.rm=T)
n=100 # number of cells in the matrix
b=min.size+c(0:n)*(max.size-min.size)/n # boundary points
y=0.5*(b[1:n]+b[2:(n+1)]) # mesh points


(lam <- Re(eigen(K$mega_matrix)$values[1]))
w.eigen <- Re(eigen(K$mega_matrix)$vectors[,1])
stable.dist <- w.eigen/sum(w.eigen)
v.eigen <- Re(eigen(t(K$mega_matrix))$vectors[,1])
